...
# class Command(BaseCommand):
#     help = "전체 더미데이터 생성 (ItemRecord 제외) - 보완판"
...
#             for menu_t in template["menus"]:
#                 dp_weight = 0.0
#                 max_discount = round(random.uniform(0.15, 0.5), 2)
#                 menu_obj = StoreMenu.objects.create(
#                     store=store,
#                     menu_name=menu_t["menu_name"],
#                     menu_image_url=menu_t["image_url"],
#                     menu_cost_price=menu_t["cost_price"],
...
#                     is_dummy=True,
#                 )
#                 menu_objs.append((menu_obj, max_discount))
...
#                 date = start_date + timedelta(days=day_offset)
#                 for hour in hours:
#                     stock = 1 if random.random() < 0.8 else 0  # 80% 확률로 재고 1
#                     StoreItem.objects.create(
#                         menu=menu,
#                         space=sms.space,
#                         store=menu.store,
