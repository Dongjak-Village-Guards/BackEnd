...
from stores.models import Store, StoreSpace, StoreMenu, StoreMenuSpace, StoreOperatingHour, StoreItem, StoreSlot
from reservations.models import Reservation, UserLike

class Command(BaseCommand):
    help = "전체 더미데이터 생성 (ItemRecord 제외)"
    ...
    def handle(self, *args, **options):
        ...
        stores = []
        for owner in owners:
            for _ in range(options['stores'] // len(owners)):
        ...
        for store in stores:
            spaces = [StoreSpace.objects.create(
                store=store,
            ...
            menus = [StoreMenu.objects.create(
                store=store,
        ...
        # Reservation / UserLike / StoreOperatingHour
