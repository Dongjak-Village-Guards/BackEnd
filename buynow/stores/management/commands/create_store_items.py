...
from stores.models import StoreItem, StoreMenuSpace, StoreSpace, StoreSlot


class Command(BaseCommand):
    help = "StoreItem과 StoreSlot을 오늘부터 8일간 배치(20개)로 추가 생성"
    ...
    def handle(self, *args, **options):
        ...
        # StoreSlot 생성
        records_to_create = []
        store_spaces = StoreSpace.objects.all()
        for space in store_spaces:
            for day_offset in range(days):
                ...
                for hour in hours:
                    record = StoreSlot(
                        space=space,
                        slot_reservation_date=date_to_create,
        ...
        if records_to_create:
            ...
            self.stdout.write(
                self.style.NOTICE(
                    f"StoreSlot 생성 완료, 마지막 배치: {len(records_to_create)}개"
                )
            )
